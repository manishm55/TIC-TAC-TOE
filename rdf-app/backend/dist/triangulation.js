...
export class TriangulationEngine {
    constructor() {
        this.lineLengthKm = 100; // Long lines for intersections
    }
    compute(bearings) {
        ...
        for (const b of bearings) {
            const start = [b.longitude, b.latitude];
                ...
                if (is.features.length > 0) {
                    for (const f of is.features) {
                        if (f.geometry && f.geometry.type === 'Point') {
                            intersections.push(f.geometry.coordinates);
                        }
                    }
                }
            }
        ...
        if (intersections.length > 0) {
            ...
            const AT = (M) => M[0].map((_, i) => M.map(row => row[i]));
            const matMul = (M, N) => M.map(row => N[0].map((_, j) => row.reduce((s, v, k) => s + v * N[k][j], 0)));
            const matVec = (M, v) => M.map(row => row.reduce((s, val, i) => s + val * v[i], 0));
            const inv2 = (M) => {
                const [[a, b], [c, d]] = M;
                const det = a * d - b * c;
                const eps = 1e-9;
        ...
        const points = [
            ...bearings.map(b => [b.longitude, b.latitude]),
            [estimatedLocation.longitude, estimatedLocation.latitude],
        ];
        const bbox = turf.bbox(turf.featureCollection(points.map(p => turf.point(p))));
        const expandDeg = 0.2; // enlarge bbox a bit
        const expanded = [bbox[0] - expandDeg, bbox[1] - expandDeg, bbox[2] + expandDeg, bbox[3] + expandDeg];
        ...
        let maxP = 0;
        for (const t of heatmapTiles)
            maxP = Math.max(maxP, t.power);
        if (maxP > 0) {
            for (const t of heatmapTiles)
                t.power = t.power / maxP;
        }
        return {
            estimatedLocation,
            confidence,
